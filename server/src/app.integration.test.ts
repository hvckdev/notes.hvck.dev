...
describe("PUT /api/note", () => {
  it("updates a note with valid secret_token", async () => {
    // Create a note first
    const postRes = await supertest(app).post("/api/note").send(testNote);
    expect(postRes.statusCode).toBe(200);
    const { note_id, secret_token } = postRes.body;
    expect(secret_token).toBeDefined();

    // Update the note
    const updatedCiphertext = Buffer.from("updated_ciphertext").toString("base64");
    const updatedHmac = Buffer.from("updated_hmac").toString("base64");
    ...
    expect(putRes.body).toHaveProperty("view_url");
    expect(putRes.body).toHaveProperty("note_id", note_id);

    // Verify the note was actually updated
    const getRes = await supertest(app).get(`/api/note/${note_id}`);
    expect(getRes.statusCode).toBe(200);
    expect(getRes.body.ciphertext).toEqual(updatedCiphertext);
    expect(getRes.body.hmac).toEqual(updatedHmac);

    // Is an update event logged?
    const updateEvents = await prisma.event.findMany({
      where: { type: EventType.UPDATE, note_id },
    });
    expect(updateEvents.length).toBe(1);
    expect(updateEvents[0].success).toBe(true);
  });

  it("rejects update with wrong secret_token", async () => {
    // Create a note first
    const postRes = await supertest(app).post("/api/note").send(testNote);
    expect(postRes.statusCode).toBe(200);
    const { note_id } = postRes.body;

    // Try to update with wrong token (must be valid base64 to pass validation)
    const putRes = await supertest(app)
      .put(`/api/note/${note_id}`)
      .send({
        ciphertext: Buffer.from("updated_ciphertext").toString("base64"),
        hmac: Buffer.from("updated_hmac").toString("base64"),
        secret_token: Buffer.from("wrong_token").toString("base64"),
      });

    expect(putRes.statusCode).toBe(401);
  });

  it("rejects update for nonexistent note", async () => {
    const putRes = await supertest(app)
      .put("/api/note/nonexistent")
      .send({
        ciphertext: Buffer.from("updated_ciphertext").toString("base64"),
        hmac: Buffer.from("updated_hmac").toString("base64"),
        secret_token: Buffer.from("any_token").toString("base64"),
      });

    expect(putRes.statusCode).toBe(404);
  });

  it("rejects update with missing ciphertext", async () => {
    // Create a note first
    const postRes = await supertest(app).post("/api/note").send(testNote);
    expect(postRes.statusCode).toBe(200);
    const { note_id, secret_token } = postRes.body;

    // Try to update without ciphertext
    const putRes = await supertest(app)
      .put(`/api/note/${note_id}`)
      .send({
        hmac: Buffer.from("updated_hmac").toString("base64"),
