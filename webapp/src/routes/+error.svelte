...
<div class="flex items-center justify-center min-h-[60vh]">
	<div class="prose max-w-2xl prose-zinc dark:prose-invert glass rounded-2xl px-6 py-8 md:px-10 md:py-10">
		...
		<div class="not-prose w-full flex justify-center mt-12">
			{#if $page.status === 404 || ($page.status === 410 && $page.error?.message === 'Note expired')}
				<img src="/expired_note.svg" alt="encrypted-art" class="w-64 md:w-80" />
			{:else if $page.status === 410 && $page.error?.message === 'Note deleted'}
				<img src="/deleted_note.svg" alt="encrypted-art" class="w-64 md:w-80" />
			{/if}
		</div>
	</div>
</div>
